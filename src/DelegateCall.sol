 ...
 * Manager contract (line 51)
 */

/**
 * Where in the code is the onERC721Received function from Vault called? You need to write out the call chain.
 * It is called from `Manager::deposit()`
 * Manager::deposit() -> Metahub::deposit() -> Assets::transferAssetToVault() -> Controller::transferAssetToVault() -> Controller::_transferAsset() -> ERC721::transferfrom()
 */

/**
 * What are address(this) and msg.sender equal to when Controller.transferAssetToVault is called?
 * address(this) - Metahub
 * msg.sender - Metahub
 */

/**
 * Will require of modifier whenAssetDepositAllowed (Vault contract) be reverted or not? Why?
 * It will not revert because msg.sender is metahub, which is executing contract through the library and in terms of Controller
 */

/**
 * Will modifier onlyDelegatecall from Controller contract correctly work out? Why?
 * No it wont because address(this) will be Metahub not _self i.e controller
 */

/**
 * How many contracts are being deployed? Which ones?
 * 4
 * metahub,manager,controller,vault
 */
